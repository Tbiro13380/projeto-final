...
<body>
                    ...
                    <div class="pt-3">
                        <div>
                            ...
                            <div class="bg-white p-2">
                                <?php
                                    $sqlCliente = "SELECT * FROM clientes WHERE id = ".$_GET['id'];           

                                    foreach($conn->query($sqlCliente)->fetchAll(PDO::FETCH_OBJ) as $lnCliente){ 

                                ?>
                                <form class="row formCliente">
                                    <input type="hidden" name="ID" value="<?php echo $lnCliente->id ?>">
                                    <div class="col-3 form-group">
                                        <label for="nome" class="pb-1 ps-1 texto-form">Nome Completo</label>
                                        <input id="nome" name="nome"  style="border-radius: 10px;" class="form-control" type="text" value="<?php echo $lnCliente->nome ?>">
                                    </div>
                                    <div class="col-3">
                                        <label for="datanasc" class="pb-1 ps-1 texto-form">Data de nascimento</label>
                                        <input id="datanasc" name="datanasc"  style="border-radius: 10px;" class="form-control data" type="text" value="<?php echo date('d/m/Y', strtotime($lnCliente->datanasc))  ?>">
                                    </div>                      
                                    <div class="col-3">
                                        <label for="genero" class="pb-1 ps-1 texto-form">Gênero</label>
                                        <select class="form-select" name="genero" id="genero">
                                            <option>Selecione</option>
                                            <option value="Masculino" <?php echo $lnCliente->genero == "Masculino" ? "selected" : "";  ?>>Masculino</option>
                                            <option value="Feminino" <?php echo $lnCliente->genero == "Feminino" ? "selected" : "";  ?>>Feminino</option>
                                            <option value="Neutro" <?php echo $lnCliente->genero == "Neutro" ? "selected" : "";  ?>>Neutro</option>
                                        </select>
                                    </div>
                                    <div class="col-3">
                                        <label for="cpf" class="pb-1 ps-1 texto-form">CPF</label>
                                        <input id="cpf" name="cpf" style="border-radius: 10px;" class="form-control cpf" type="text" value="<?php echo $lnCliente->cpf ?>">
                                    </div>
                                    <div class="col-3 pt-3">
                                        <label for="celular" class="pb-1 ps-1 texto-form">Celular</label>
                                        <input id="celular" name="celular" style="border-radius: 10px;" class="form-control cell" type="text" value="<?php echo $lnCliente->celular ?>">
                                    </div>
                                    <div class="col-3 pt-3">
                                        <label for="email" class="pb-1 ps-1 texto-form">E-mail</label>
                                        <input id="email" name="email" style="border-radius: 10px;" class="form-control" type="text" value="<?php echo $lnCliente->email ?>" >
                                    </div>
                                    <div class="col-3 pt-3">
                                        <label for="senha" class="pb-1 ps-1 texto-form">Senha</label>
                                        <input id="senha" name="senha" style="border-radius: 10px;" class="form-control" type="password" value="******">
                                    </div>
                                    <div class="col-3 pt-3">
                                        <label for="status" class="pb-1 ps-1 texto-form">Status</label>
                                        <select class="form-select" name="status" id="status">
                                            <option value="Ativo" <?php echo $lnCliente->status == "Ativo" ? "selected" : "";  ?>>Ativo</option>
                                            <option value="Inativo" <?php echo $lnCliente->status == "Inativo" ? "selected" : "";  ?>>Inativo</option>
                                        </select>
                                    </div>
    ...
    <script>
        ...
        $('#salvar').on("click", function(e){
            ...
            $.ajax({
               type: "POST",
                url: 'posts/postCliente.php',
                data: $('.formCliente').serialize(),
                dataType: 'json',

                success: function(response) {
                    if(response.status) {
                        Swal.fire({title: 'Sucesso', html: response.message, icon: 'success'}).then(() => {location.reload()});
                    } else {
